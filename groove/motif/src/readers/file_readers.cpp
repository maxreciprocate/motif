...
void read_markers(MARKERS_DATA* markers_data, std::ifstream &in) {
    uint64_t sum_of_all_chars = 0;
    uint32_t longest_marker_len = 0;
    std::string line;

    // MARKERS_DATA markersData = *markers_data;
    while (std::getline(in, line)) {
        markers_data->markers.emplace_back(
            line.begin() + line.find(',') + 1, line.end()
        );
    ...
    markers_data->sum_of_all_chars = sum_of_all_chars;
    markers_data->longest_marker_len = longest_marker_len;
